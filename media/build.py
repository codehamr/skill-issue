#!/usr/bin/env python3
"""Build the native game and regenerate all media, reporting total wall time."""
import argparse
import os
...
def main():
    ...
    try:
        ...
        subprocess.run(shlex.split(args.make) + ["--no-print-directory", "-e", "build/game"],
        ...
        print("media: " + str(error), file=sys.stderr)
    finally:
        elapsed = time.monotonic() - started
        hours, remainder = divmod(int(elapsed), 3600)
        minutes, seconds = divmod(remainder, 60)
        print("media: elapsed %02d:%02d:%02d (%.1f s), %s" %
