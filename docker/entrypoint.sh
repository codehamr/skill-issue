...
# Pass the configured server overrides. Bot-fill and idle-timeout use the
# downloaded binary's defaults; this supervisor does not override them.
...
# stats.txt is the aggregated host-readable report: a byte-count header and
# the log pass through report.awk, then a successful render replaces the file.
stats() {
  { wc -c < "$LOG"; cat "$LOG"; } \
...
# for empty ones. This is a log-based emptiness heuristic; a stalled or silent
# process can also have no recent digest.
srv_empty() {
